    ...
    def _get_data(self):
        ...
        for lang in (self.config.source_lang, self.config.target_lang):
            ...
            if not os.path.exists(preprocessed_data_path):
                ...
                # self.dataset[lang] = self.dataset[lang][:100]   # for debug!!!
                self.preprocessors[lang].load_tokenizer_state(tokenizer_path_to_load)

            if self.set_type == SetType.validation:
                # smaller dataset for faster validation
                self.dataset[lang] = self.dataset[lang][:int(len(self.dataset[lang])/2)]
